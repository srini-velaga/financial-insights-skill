...
from fin_insights.session import cached_query
...
def get_monthly_spending_by_category(conn: duckdb.DuckDBPyConnection, months: int | None = None) -> list[dict]:
    ...
    cache_key = f"monthly_spending_by_category:{months}"
    ...
    return cached_query(conn, cache_key, "monthly_spending_by_category", params, _query)
...
def get_month_over_month(conn: duckdb.DuckDBPyConnection, months: int | None = None) -> list[dict]:
    ...
    params = {"months": months}
    cache_key = f"month_over_month:{months}"
    ...
    return cached_query(conn, cache_key, "month_over_month", params, _query)
...
def get_top_merchants(conn: duckdb.DuckDBPyConnection, limit: int = 10, months: int | None = None) -> list[dict]:
    ...
    cache_key = f"top_merchants:{limit}:{months}"
    ...
    return cached_query(conn, cache_key, "top_merchants", params, _query)
...
def get_category_breakdown(
    ...
    cache_params = {"month": month, "year": year}
    cache_key = f"category_breakdown:{month}:{year}"
    ...
    return cached_query(conn, cache_key, "category_breakdown", cache_params, _query)
...
def get_cashflow(conn: duckdb.DuckDBPyConnection, months: int | None = None) -> list[dict]:
    ...
    return cached_query(conn, cache_key, "cashflow", params, _query)
...
def get_spending_by_card(conn: duckdb.DuckDBPyConnection, months: int | None = None) -> list[dict]:
    ...
    params = {"months": months}
    cache_key = f"spending_by_card:{months}"
    ...
    return cached_query(conn, cache_key, "spending_by_card", params, _query)
